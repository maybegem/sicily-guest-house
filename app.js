...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ------------------------------------------------------------------
  // 2. WhatsApp Form Handler (Ultra Clean Text Format)
  // ------------------------------------------------------------------
  ...
  if (bookingForm) {
    bookingForm.addEventListener('submit', (e) => {
      ...
      // Clean string formatting for guests & room
      let guestsCount = guestsSelect ? guestsSelect.value : '2 persone';
      let selectedRoom = roomSelect ? roomSelect.value : 'qualsiasi camera';

      // Clean plain-text WhatsApp message
      const message = `Ciao Sicily Guest House!\n` +
                      `Vorrei verificare la disponibilita per un soggiorno a Palermo:\n\n` +
                      `- Check-in: ${checkinFormatted}\n` +
                      `- Check-out: ${checkoutFormatted} (${nights} nott${nights === 1 ? 'e' : 'i'})\n` +
                      `- Ospiti: ${guestsCount}\n` +
                      `- Sistemazione: ${selectedRoom}\n\n` +
                      `Potete darmi conferma sulla disponibilita e sui prezzi? Grazie!`;
